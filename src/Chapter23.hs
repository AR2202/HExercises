...
mkStore livingCells ind = store checkFunction ind
  where
    checkFunction = checkCell livingCells

--logic of updating one cell-------------------------
neighbours (x, y) =
  filter (/= (x, y)) [(x1, y1) | x1 <- [x - 1 .. x + 1], y1 <- [y - 1 .. y + 1]]
...
step (cells, living) = (newCells, newLiving)
  where
    newCells = updateStore living $ extend neighboursToCellState cells
    newLiving = updateLivingCells living newCells

updateLivingCells :: [Coord] -> Store Coord Cell -> [Coord]
updateLivingCells living currentCells =
  filter (\x -> (peek x currentCells) == Alive) $
  nub $ living ++ livingNeighbours
  where
    livingNeighbours = living >>= neighbours

updateStore currentLiving currentCells = mkStore newLiving ind
  where
    newLiving = updateLivingCells currentLiving currentCells
...
displayGrid coords cellstore =
  (map . map) display1 $ (map . map) flippedPeek coords
  where
    flippedPeek = flip peek $ cellstore

corners2grid :: Coord -> Coord -> [[Coord]]
corners2grid (x1, y1) (x2, y2) =
  groupBy (\x y -> snd x == snd y) $ [(x, y) | y <- [y1 .. y2], x <- [x1 .. x2]]

printGrid :: Coord -> Coord -> Store Coord Cell -> IO ()
printGrid lefttop rightbottom cellstore = mapM_ putStrLn $ grid
  where
    grid = displayGrid coordlist cellstore
...
printGridCLI numsteps lefttop rightbottom startingdata =
  ...
  where
    disp = printGrid lefttop rightbottom . fst
...
cells2picture cellwidth coords cellstore f = pictures $ map cell2Pic coords
  where
    cell2Pic coord =
      placePicture cellwidth (cell2picturefun coord) (transformedCoord coord)
    cell2picturefun coord = cell2picture $ flippedPeek coord
    flippedPeek = flip peek $ cellstore
    transformedCoord coord = cellcoord2picturecoord f coord

placePicture cellwidth picturefun (xcoord, ycoord) =
  translate x y (picturefun cellwidth)
  where
    x = xcoord * cellwidth
    y = ycoord * cellwidth
...
makeTransformFunc lefttop rightbottom =
  \(x, y) -> (fromIntegral x - leftx - halfn, fromIntegral y - lefty - halfm)
  where
    leftx = fromIntegral $ fst lefttop
...
step2pic initialstates numstep = cells2picture cellw coord newstore f
  where
    newstore = fst $(iterate step startingdata) !! numstep
...
window n m =
  InWindow "Game Of Life" (screenWidth n m, screenHeight n m) (100, 100)

--
--animateGrid :: IO ()
animateGrid lefttop rightbottom livingStart =
  animate win backgroundColor floatToPic
  where
    floatToPic = \time -> step2pic initialstates $ time2step time
    win = window n m
    n = corners2n lefttop rightbottom
    m = corners2m lefttop rightbottom
    initialstates =
      InitialStates
        { cellwidth = cellWidth n m
