...
data Polynomial
  = Poly [Int] -- Polynomial literals represented as List of coefficients - empty list is Zero and [1] is One of the Ring
...
evalPolynomial (PAdd p1 p2) =
  sumPolynomials (evalPolynomial p1) (evalPolynomial p2)
evalPolynomial (PMult p1 p2) =
  multiplyPolynomials (evalPolynomial p1) (evalPolynomial p2)
  where
    multiplyPolynomials [] _ = []
    multiplyPolynomials (x:xs) ys =
      sumPolynomials (fmap (x *) ys) (multiplyPolynomials xs (0 : ys))
...
era' (numprimes, p:ns) =
  if numprimes == 0
    then Nothing
    else Just (p, (numprimes - 1, filter (notdiv p) ns))
  where
