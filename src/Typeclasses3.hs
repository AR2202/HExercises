...
data Adult1 =
  Adult1 String String Int
  deriving (Show, Eq, Ord)

data Adult2 =
  Adult2 String String Int
  deriving (Show, Eq)

instance Ord Adult2 where
  compare (Adult2 firstname lastname _) (Adult2 firstname2 lastname2 _) =
    case compare lastname lastname2 of
      EQ -> compare firstname firstname2
      _  -> compare lastname lastname2

-- Derive both 'Ord' and 'Read' for this type.
newtype InterestRate =
  InterestRate Double
  deriving (Eq, Show, Ord, Read)--deriving newtype Read
...
returnHigherInterestRate :: (Show a, Ord b) => (a, b) -> (a, b) -> String
returnHigherInterestRate (p1, i1) (p2, i2) =
  "'" ++ show p ++ "' has a higher interest rate!"
  where
    p =
      if i1 > i2
        then p1
...
main =
  defaultMain $
  testGroup "Typeclasses3" $
  [ testCase "Ordering Adults 1" $
    sort
      [ Adult1 "Zach" "Whittaker" 31
      , Adult1 "John" "Smith" 45
      , Adult1 "Thomas" "Allen" 46
      ] @?=
    [ Adult1 "John" "Smith" 45
    , Adult1 "Thomas" "Allen" 46
    , Adult1 "Zach" "Whittaker" 31
  ...
  , testCase "Ordering Adults 2" $
    sort
      [ Adult2 "Zach" "Whittaker" 31
      , Adult2 "John" "Smith" 45
      , Adult2 "Thomas" "Allen" 46
      ] @?=
    [ Adult2 "Thomas" "Allen" 46
    , Adult2 "John" "Smith" 45
    , Adult2 "Zach" "Whittaker" 31
    ]
  , testCase "Read Interest Rate" $
    map read ["InterestRate 0.5", "InterestRate 0.3", "InterestRate 0.788"] @?=
    [InterestRate 0.5, InterestRate 0.3, InterestRate 0.788]
  , testCase "Higher Interest Rate 1" $
    returnHigherInterestRate ("John", 0.3) ("Tom", 0.03) @?=
    "'\"John\"' has a higher interest rate!"
  , testCase "Higher Interest Rate 1" $
    returnHigherInterestRate
      (Adult1 "John" "Smith" 15, InterestRate 0.07)
      (Adult1 "Tom" "Allen" 18, InterestRate 0.1) @?=
