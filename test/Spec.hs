  ...
  hspec $ describe "fizzes" $ do
    ...
    describe "buzzes" $ do
      ...
      describe "fizzbuzzes" $ do
        it
          "should have fizzbuzz as every fifteenth element"
          $property
          prop_fizzbuzz_every_fifteenth
        describe "numfizzbuzz" $ do
          it "should have Strings, not Maybe" $property prop_numfizzbuzz_string
          it
            "should have number if it is not divisible by 3 or 5"
            $property
            prop_numfizzbuzz_numb
          describe "neighbours" $ do
            it
              "should not contain the element itself"
              $property
              prop_neighbours_noSelf
            it "should be symmetric" $property prop_neighbours_symmetric
...
prop_fizzbuzz_every_fifteenth (NonNegative x) =
  fizzbuzzes !! (15 * x + 14) == Just "fizzbuzz"

prop_numfizzbuzz_string :: NonNegative Int -> Bool
prop_numfizzbuzz_string (NonNegative x) =
  numfizzbuzz !! (15 * x + 14) == "fizzbuzz"

prop_numfizzbuzz_numb :: NonNegative Int -> Property
prop_numfizzbuzz_numb (NonNegative x) =
  x `mod` 5 > 0 && x `mod` 3 > 0 ==> numfizzbuzz !! (x - 1) == show x
...
prop_neighbours_symmetric coord =
  forAll (choose (0, length (neighbours coord) - 1)) $ \n ->
